...
The classic group ideation method. Rules: (1)~defer all judgment---no criticizing ideas during generation, (2)~encourage wild ideas---they trigger useful associations, (3)~build on others' ideas (``yes, and\ldots'' not ``no, but\ldots''), (4)~go for quantity over quality. A productive brainstorming session generates 30--50 ideas in 20 minutes. Most will be impractical; that is the point. The few gems are worth the pile of duds.
...
\subsection{SCAMPER}
A structured creativity prompt that forces you to transform existing solutions. For each existing solution or concept, systematically ask:
...
SCAMPER is particularly useful when you have one decent concept but need alternatives for the decision matrix. Apply each letter to your existing concept and see what emerges.

\subsection{Morphological Chart}
The morphological chart is the most systematic ideation technique. Decompose the system into independent functions (rows) and list multiple solution approaches for each function (columns). Then systematically combine solutions across functions to generate novel system-level concepts.
...
Study how nature solves analogous problems. Natural systems have been optimized by billions of years of evolution for efficiency, resilience, and adaptability. Examples: gecko feet for reversible adhesion, shark skin for drag reduction, termite mounds for passive ventilation, bone structure for lightweight high-strength materials, spider silk for tensile strength. The AskNature database (\url{https://asknature.org}) catalogs biological strategies organized by function. Biomimicry is particularly useful for structural, thermal, fluid, and materials challenges.
...
The Pugh matrix is a quick, qualitative screening tool for reducing a large set of concepts to a manageable shortlist. Steps:
...
For each component or function in your design: how can it fail (failure mode)? What happens to the system if it fails (effect)? Three ratings:
\begin{itemize}[nosep,leftmargin=1.5em]
...
\textbf{SCAMPER}: a structured modification technique applied to an existing product or concept. For each letter, ask: \textbf{S}ubstitute (what component or material can be replaced?), \textbf{C}ombine (can two functions be merged into one component?), \textbf{A}dapt (can a solution from a different domain be applied here?), \textbf{M}odify/Magnify/Minimize (can dimensions, power, speed, or complexity be changed?), \textbf{P}ut to another use (can the system serve a secondary purpose?), \textbf{E}liminate (what can be removed without losing core functionality?), \textbf{R}everse/Rearrange (can the sequence, orientation, or layout be inverted?). Apply SCAMPER to your top 2--3 concepts from initial brainstorming to generate 10--20 variations.

\textbf{Biomimicry}: nature has solved most engineering problems already. Ask: ``How does nature handle this function?'' Examples: thermal regulation (elephant ears = large-surface-area heat exchangers), structural efficiency (bone = optimized load-bearing with minimum material), water collection (Namibian beetle = fog harvesting through surface wettability gradients), vibration damping (woodpecker skull = layered energy absorption). Biomimicry produces concepts that are often lighter, more efficient, and more elegant than conventional engineering solutions.
