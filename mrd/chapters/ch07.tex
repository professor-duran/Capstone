...
\item \textbf{Product Backlog}: all known work items, prioritized by client value (MoSCoW~[10]).
...
At each semester's start, set SMART goals~[38] with PA and client agreement: Specific, Measurable, Achievable, Relevant, Time-bound. Examples:
\begin{itemize}[nosep,leftmargin=1.5em]
...
For each risk in the register, select one or more mitigation strategies~[8]:
