...
\textbf{Lockout/tagout (LOTO)}~[22],~[23]: when working on energized equipment, disconnect the power source and apply a physical lock to prevent re-energization. In capstone, this means disconnecting the battery or unplugging the power supply before modifying wiring, not just turning off the switch.
...
If a cell swells, hisses, smokes, or begins to heat uncontrollably: \textbf{clear the area immediately}. If safe to do so, move the battery outdoors to a metal container (e.g., ammo can or metal bucket with sand) using insulated tools. Call 911 if fire occurs. For lithium-ion or LiPo battery fires: use water in large quantities to cool the cells and prevent thermal runaway propagation. Do \textbf{not} use a Class~D extinguisher (those are designed for combustible metals such as magnesium or sodium, not lithium-ion cells). For small fires, an ABC dry chemical extinguisher may also be used.
...
Power tools, CNC machines, and compressed air tools can produce noise levels above 85\,dBA~[43], which causes permanent hearing damage with prolonged exposure. Wear hearing protection (earplugs or earmuffs) when working near loud equipment. If you must shout to be heard at arm's length, the noise exceeds 85\,dBA.
...
\textbf{Lightning}: Colorado consistently ranks among the states with the highest lightning fatality rates per capita (NOAA Storm Data). Afternoon thunderstorms are common May--September. If you hear thunder, seek shelter immediately. The 30/30 rule: if the time between lightning flash and thunder is $<$30 seconds, seek shelter and stay sheltered for 30 minutes after the last thunder.
...
\item Compliance with applicable safety standards (IEC~61010~[21], OSHA requirements~[22], NFPA~70E~[23], competition rules, site-specific requirements).
\end{itemize}
...
\textbf{Emergency response}: if a cell swells, hisses, smokes, or begins to heat uncontrollably: (1)~clear the area immediately, (2)~if safe to do so, move the battery to a metal container with sand using insulated tools, (3)~call 911 if fire occurs, (4)~for lithium-ion or LiPo fires, use water in large quantities to cool the cells and prevent thermal runaway propagation; do \textbf{not} use a Class~D extinguisher (those are for combustible metals, not Li-ion cells); for small fires, an ABC dry chemical extinguisher may also be used, (5)~ventilate the area (lithium battery fires produce toxic hydrogen fluoride gas).
