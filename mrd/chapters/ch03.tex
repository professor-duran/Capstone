...
The time you invest in writing clear, testable, traceable requirements during Sprint~1--2 will save you ten times that effort during build and test. Every hour spent refining requirements saves 10--100 hours of rework downstream~[36]. This is not a platitude; it is an empirically observed pattern across decades of engineering project data.
...
\item \textbf{Verification method}: how you will prove compliance; Test (T), Analysis (A), Inspection (I), or Demonstration (D)~[2]. Assign the method when you write the requirement.
\end{enumerate}
...
Not all requirements are equally critical. Classify each requirement using the MoSCoW framework~[10]:
...
\begin{tipbox}[Turning ``Accessible'' into Testable Requirements]
``The device shall be accessible'' is not a testable requirement. Universal Design principles (Ronald Mace, NC State) provide a framework for translating accessibility goals into measurable, verifiable SHALL statements. Instead of vague intent, write specific requirements:
\begin{itemize}[nosep,leftmargin=1.5em]
\item \textbf{Vague}: ``The device shall be accessible to users with disabilities.''
\item \textbf{Testable}: ``The device shall be operable with one hand and require no more than 5\,lbf of force on any control.'' (TAID: Test)
\item \textbf{Testable}: ``All status indicators shall provide both visual and audible output.'' (TAID: Demonstration)
\item \textbf{Testable}: ``The touchscreen interface shall maintain a minimum contrast ratio of 4.5:1 per WCAG~2.1 Level~AA.'' (TAID: Inspection)
\item \textbf{Testable}: ``All controls shall be reachable within 15--48~in.\ above finished floor from a seated position per ADA reach range.'' (TAID: Inspection)
\end{itemize}
Apply this pattern to every accessibility-related need: identify the specific barrier (mobility, vision, hearing, cognition), then write a requirement with a measurable threshold and a verification method. These requirements belong in the non-functional section of the SDRD alongside environmental and safety constraints.
\end{tipbox}

\section{The System Design Requirements Document (SDRD)}
The SDRD is the single authoritative document that captures all system-level requirements~[33]. It is the foundational engineering document for the entire project. Standard structure:
...
The VCRM~[34] extends the RTM by adding test procedure references and results. For each requirement, the VCRM records:
\begin{itemize}[nosep,leftmargin=1.5em]
