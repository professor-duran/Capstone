...
These terms are frequently confused, even by practicing engineers. The distinction matters because they answer fundamentally different questions and require different evidence.
...
Not every requirement is verified by running a physical test. The four methods provide a spectrum of rigor and cost:
...
A test procedure is a step-by-step recipe that allows someone who did not design the system to verify a requirement independently. Every test procedure should include:
...
\textbf{Data recording}: what to record (raw measurements, calculated values, environmental conditions), in what format (table, datasheet, electronic log), and where to store it (project repository, lab notebook).
...
\section{The Test Procedure Template}
For each requirement verified by Test (T), write a formal test procedure:

\textbf{Test ID}: TP-XXX (matching the requirement ID, e.g., TP-PR-004 tests PR-004).

\textbf{Objective}: ``Verify that the system maintains temperature within $\pm$1.0\,\degC{} of the setpoint over the operating range of 20--200\,\degC{}.''

\textbf{Equipment required}: list every piece of test equipment with model numbers and calibration status. ``Fluke 179 multimeter (calibrated 2026-01-15), Type~K thermocouple (NIST-traceable reference), Bench power supply (BK Precision 1672A).''

\textbf{Test setup}: describe (and photograph) the physical configuration. How is the device under test connected to the measurement equipment? What is the ambient environment?

\textbf{Procedure}: numbered steps that anyone on the team can follow to reproduce the test. Include: setup steps, measurement steps, data recording instructions, and cleanup steps.

\textbf{Pass/fail criteria}: derived directly from the requirement. ``PASS: measured error $\leq \pm$1.0\,\degC{} at all 5 test points. FAIL: measured error $> \pm$1.0\,\degC{} at any test point.''

\textbf{Data recording}: specify what data to record, in what format, and where to store it. Raw data files should be saved in the test data directory of the project repository with a naming convention: \texttt{TP-PR-004\_run01\_2026-04-10.csv}.
