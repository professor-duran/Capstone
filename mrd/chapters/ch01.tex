...
\subsection{Your Role: A Contracted Design Firm}
Think of your capstone team not as students completing an assignment, but as a contracted design firm that has been hired to solve a problem. Your client has a need---perhaps a process that is too slow, a system that does not exist, a site that must be redesigned---but they do not have the engineering expertise to solve it themselves. That is why they hired you. If they already knew the solution, they would not need an engineer.

This framing has practical consequences for how you operate. You are not order-takers waiting for the client to tell you what to build and how to build it. You are the engineering experts. The client provides the need, the constraints, and the context; you provide the analysis, the design, and the technical direction. When you sit down with your client, you should be leading the conversation: proposing approaches, presenting trade studies, recommending a path forward, and asking targeted questions to fill gaps in your understanding. The client validates your direction; they do not dictate it.

In practice, this means you will frequently encounter situations where the information you need is incomplete, ambiguous, or contradictory. A client may say ``make it reliable'' without specifying a mean time between failures. They may describe an operating environment in general terms that leave critical parameters undefined. They may change priorities between meetings. This is not a failure of the client; it is the normal condition of engineering consulting. Your job is to \emph{make reasonable assumptions, document them, present them to the client for validation, and be prepared to iterate when new information surfaces}. The ability to move forward with incomplete information---making defensible decisions rather than waiting for perfect clarity---is one of the most important professional skills you will develop in capstone.

\begin{keyconceptbox}[Lead the Design, Validate with the Client]
Your team owns the technical direction of the project. When information is missing, make an engineering assumption and document it. When multiple approaches are viable, conduct a trade study and recommend one. When the client's stated preference conflicts with engineering analysis, present the evidence and have a professional conversation about the trade-off. Bring your client solutions and recommendations, not open-ended questions. The pattern is: \textbf{decide $\to$ document $\to$ validate $\to$ iterate}. Comfort with ambiguity is not optional; it is the job.
\end{keyconceptbox}
