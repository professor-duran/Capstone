...
\item \textbf{Accredited degree}: earn an ABET-accredited engineering degree. You are completing this requirement now.
\item \textbf{FE exam}: pass the Fundamentals of Engineering exam, administered by NCEES. The FE is typically taken during your senior year or shortly after graduation. It is a 6-hour, 110-question computer-based exam covering mathematics, sciences, and engineering fundamentals. Passing the FE makes you an \textbf{Engineer Intern (EI)} or \textbf{Engineer-in-Training (EIT)}.
...
\textbf{Challenger (1986)}: Morton Thiokol engineers warned that O-ring seals would fail at low temperatures. Management overruled the recommendation. Seven crew members died. Lesson: the obligation to public safety overrides schedule pressure and management hierarchy.

\textbf{Flint Water Crisis (2014--)}: cost-cutting decisions to change water sources without adequate corrosion control exposed a predominantly low-income, majority-Black community to lead contamination. Engineers who raised concerns were overruled or ignored. Lesson: equity and environmental justice are engineering responsibilities, not just political considerations.

\textbf{Boeing 737 MAX (2018--2019)}: design decisions to minimize pilot training requirements for a new flight control system (MCAS) contributed to two crashes and 346 deaths. Lesson: when safety-critical systems are designed to minimize cost rather than maximize safety, the consequences can be catastrophic.

\textbf{Hyatt Regency Walkway Collapse (1981)}: a structural connection detail was changed during construction without adequate engineering review. 114 people died. Lesson: every change to a safety-critical design must be reviewed by a qualified engineer, no matter how minor it appears.
...
\section{The Course Concerns Chain of Command}
If you have concerns about grading, team dynamics, PA interactions, or course policies:
\begin{enumerate}[nosep,leftmargin=1.5em]
\item \textbf{Project Advisor (PA)}: first point of contact for all project-related concerns.
\item \textbf{Course Lead Departmental Faculty}: if unresolved with PA.
\item \textbf{Capstone Program Director}: Dr.\ Kristy Csavina (kcsavina@mines.edu).
\item \textbf{Dean}: for concerns unresolvable at the program level.
\end{enumerate}
Most concerns are resolved at Step~1. If you skip steps, you will be directed back.
...
Capstone Design is the primary assessment vehicle for several ABET Student Outcomes. Your work in capstone directly demonstrates your competence in these areas:
