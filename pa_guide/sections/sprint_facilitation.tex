...
\subsection{Sprint 2: Concept Generation, Trade Study}
\textit{Weeks 5--6 $|$ Course Text: Chapters 4, 20}

\textbf{Health indicators:} The team has generated a broad set of concepts ($\geq$15 ideas before down-selection). A Pugh matrix screens initial concepts; top candidates advance to a weighted decision matrix with sensitivity analysis. Background research supports concept feasibility.

\begin{redflag}
Jumping to a single concept without exploring alternatives, or a decision matrix with unweighted criteria and no sensitivity analysis. This is the single most common Sprint~2 error.
\end{redflag}

\textbf{Facilitation technique---Running a brainstorming session:} Set a 15-minute timer and enforce three rules: no criticism, no filtering, and no ownership of ideas. The goal is volume ($\geq$15 ideas). Write every idea on a whiteboard or sticky notes so the team sees the full space before converging. Only after the divergent phase ends should the team begin grouping, combining, and screening ideas. If the team stalls, prompt with category forcing: ``What would a purely mechanical solution look like? A purely electrical one? A software-only one?''

\textbf{Facilitation technique---Pugh matrix review:} The most common failure mode of a Pugh matrix is confirmation bias: students score their favorite concept high on every criterion because they have already decided. Counter this by assigning a different team member to advocate for each concept during scoring, and by requiring written one-sentence justifications for every cell. If all concepts score identically on a criterion, the criterion is not discriminating and should be revisited.

\textbf{Facilitation technique---Pushing past the first idea:} When a team converges prematurely on a single concept, use constraint removal: ``What would you build if cost were not a constraint?'' or constraint inversion: ``What if you could not use [their current approach]---what would you try instead?'' This forces the team to explore the design space rather than anchoring on the first workable idea. A team that cannot articulate why their chosen concept is better than two specific alternatives has not done enough concept generation.

\textbf{Key deliverables:} Concept screening (Pugh matrix), weighted decision matrix, trade study documentation, Career Day preparation.
