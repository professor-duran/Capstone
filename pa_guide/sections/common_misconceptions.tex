...
\begin{redflag}
\textbf{Misconception:} Accessibility does not apply to our project.\\
\textit{How it appears:} The SDRD contains no accessibility requirements, no consideration of ADA compliance, and the team assumes a narrow user population without justification.\\
\textit{How to address:} Ask: ``Could someone with one hand use this? Could someone in a wheelchair access the controls? Could someone with low vision read the display?'' Even projects with specialized end users benefit from accessible design. Accessibility is a design requirement, not an afterthought.
\end{redflag}

\subsection{Verification and Validation}

\begin{redflag}
\textbf{Misconception:} ``We tested it and it works'' constitutes verification.\\
\textit{How it appears:} The team demonstrates the system functioning once in ideal conditions and considers the requirement verified. There is no written test procedure, no pass/fail criteria defined before the test, and no recorded data---only a live demo or a video.\\
\textit{How to address:} Ask: ``Show me the VCRM disposition for each requirement. Where is the test procedure, the recorded data, and the pass/fail threshold?'' A demonstration is not a test. Verification requires a documented procedure, pre-defined acceptance criteria, and traceable evidence.
\end{redflag}

\begin{redflag}
\textbf{Misconception:} Sustainability means recycling.\\
\textit{How it appears:} The sustainability section mentions using recyclable materials or 3D-printed PLA but ignores energy consumption during operation, embodied energy in manufacturing, material toxicity, and end-of-life planning. The lifecycle assessment, if present, is superficial.\\
\textit{How to address:} Ask: ``What is the energy consumption of your system over its expected lifetime? What happens to this device at end of life---can it be disassembled, repaired, or recycled? How did material selection account for environmental impact beyond recyclability?'' Sustainability analysis must span the full lifecycle, not just disposal.
\end{redflag}
